...
t.test(as.numeric(d_gender[1,2:8]),
            as.numeric(d_gender[1,9:15]), 
            paired = TRUE)
wilcox.test(as.numeric(d_gender[1,2:8]),
            as.numeric(d_gender[1,9:15]), 
            paired = TRUE,
            conf.int = TRUE)
t.test(as.numeric(d_gender[2,2:8]),
            as.numeric(d_gender[2,9:15]), 
            paired = TRUE)
wilcox.test(as.numeric(d_gender[2,2:8]),
            as.numeric(d_gender[2,9:15]), 
            paired = TRUE,
            conf.int = TRUE)
t.test(as.numeric(d_gender[3,2:8]),
            as.numeric(d_gender[3,9:15]), 
            paired = TRUE)
...
t.test(as.numeric(d_gender[1,16:22]),
            as.numeric(d_gender[1,9:15]), 
            paired = TRUE)
wilcox.test(as.numeric(d_gender[1,16:22]),
            ...
            paired = TRUE,
            conf.int = TRUE)
t.test(as.numeric(d_gender[2,16:22]),
            as.numeric(d_gender[2,9:15]), 
            paired = TRUE)
wilcox.test(as.numeric(d_gender[2,16:22]),
            ...
            paired = TRUE,
            conf.int = TRUE)
t.test(as.numeric(d_gender[3,16:22]),
            as.numeric(d_gender[3,9:15]), 
            paired = TRUE)
...
country_cont1 <- subset(d_country_cont1, 
                        d_country_cont1$Country %in% common_country)
country_treat <- subset(d_country_treat, 
                        d_country_treat$Country %in% common_country)
...
wilcox.test(country$Sess.control1,
            country$Sess.treat, 
            paired = T,
            conf.int = TRUE)
t.test(country$Sess.control1,
       country$Sess.treat, 
       paired = T)

## Treatment v.s. Control 2 
common_country <- intersect(d_country_cont2$Country,d_country_treat$Country)
country_cont2 <- subset(d_country_cont2, 
                        d_country_cont2$Country %in% common_country)
country_treat <- subset(d_country_treat, 
                        d_country_treat$Country %in% common_country)
...
wilcox.test(country$Sess.control2,
            country$Sess.treat, 
            paired = T,
            conf.int = TRUE)
t.test(country$Sess.control2,
       country$Sess.treat, 
       paired = T)
...
lang_cont1 <- subset(d_lang_cont1, 
                     d_lang_cont1$Language %in% common_lang)
lang_treat <- subset(d_lang_treat, 
                     d_lang_treat$Language %in% common_lang)
...
wilcox.test(lang$Sess.control1,
            lang$Sess.treat, 
            paired = T,
            conf.int = TRUE)
t.test(lang$Sess.control1,
       lang$Sess.treat, 
       paired = T)

## Treatment v.s. Control 2
common_lang <- intersect(d_lang_cont2$Language,d_lang_treat$Language)
lang_cont2 <- subset(d_lang_cont2, 
                     d_lang_cont2$Language %in% common_lang)
lang_treat <- subset(d_lang_treat, 
                     d_lang_treat$Language %in% common_lang)
...
wilcox.test(lang$Sess.control2,
            lang$Sess.treat, 
            paired = T,
            conf.int = TRUE)
t.test(lang$Sess.control2,
       lang$Sess.treat, 
       paired = T)
...
### 3.1 Revenue and QTY of items sold

```{r}
## Sales Summary
sales_summary <- data.frame(
  week=c("C1","TR","C2"),
  revenue=c(sum(SO_cont1$OrderValue),
            sum(SO_treat$OrderValue),
            sum(SO_cont2$OrderValue)),
  nrorders=c(length(SO_cont1$OrderID),
             length(SO_treat$OrderID),
             length(SO_cont2$OrderID)),
  qty=c(sum(SO_cont1$NrItems),
        sum(SO_treat$NrItems),
        sum(SO_cont2$NrItems)
  )
)
sales_summary

## QTY of items sold per item price group
qty_treat <- c(47,0,4,4,0)
qty_cont1 <- c(23,0,1,4,1)
qty_cont2 <- c(12,0,4,1,1)
wilcox.test(qty_treat, qty_cont1, paired = T)
wilcox.test(qty_treat, qty_cont2, paired = T)
```
...
coeftest(m1_ordervalue, cov=vcov)
## t-test
t.test(SO_cont1$OrderValue,SO_treat$OrderValue)
## non-parametric test
...
plot(density(dist_sharpnull_ordervalue), 
     main="Sharp null hypothesis dist. of Sales (TR vs C1)")
abline(v=ate, col="red")
mean(abs(dist_sharpnull_ordervalue)>=abs(ate)) # p value
sd(dist_sharpnull_ordervalue) # standard error from sharp null
...
m3_ordervalue$vcov <- vcov(m3_ordervalue)
coeftest(m3_ordervalue, m3_ordervalue$vcov)
```
...
## t-test
t.test(SO_cont2$OrderValue,SO_treat$OrderValue)
## non-parametric test
...
plot(density(dist_sharpnull_ordervalue), 
     main="Sharp null hypothesis dist. of Sales (TR vs C2)")
abline(v=ate, col="red")
mean(abs(dist_sharpnull_ordervalue)>=abs(ate)) # p value
sd(dist_sharpnull_ordervalue) # standard error from sharp null
...
m6_ordervalue$vcov <- vcov(m6_ordervalue)
coeftest(m6_ordervalue, m6_ordervalue$vcov)
```
...
SO Estimate across Weeks

```{r}
SO_summary <- data.frame(
  week=c("C1","TR","C2"),
  SO=c(mean(SO_cont1$OrderValue),
       mean(SO_treat$OrderValue),
       mean(SO_cont2$OrderValue)),
  nrorders=c(length(SO_cont1$OrderID),
             length(SO_treat$OrderID),
             length(SO_cont2$OrderID))
  )
...
## 3.3 Price per Unique Item per Order
...
plot(density(dist), main="Sharp null hypothesis dist. of Sales (TR vs C1)")
abline(v=ate,col="red")
mean(abs(ate)<=abs(dist)) # p-value
sd(dist) # standard error from sharp null
...
plot(density(dist), main="Sharp null hypothesis dist. of Sales (TR vs C2)")
abline(v=ate,col="red")
mean(abs(ate)<=abs(dist)) # p-value
sd(dist) # standard error from sharp null
...
plot(density(dist), main="Baseline Sales Comparison (C1 vs C2)")
abline(v=ate,col="red")
mean(abs(ate)<=abs(dist)) # p-value
sd(dist) # standard error from sharp null
```
...
### 3.4 Sales Value per Buyer / Order Value per Buyer (OVB)
...
## Treatment v.s. Control 2
SVB_treat <- aggregate(x=SO_treat$OrderValue,by=list(SO_treat$BuyerID), FUN=sum)
SVB_cont2 <- aggregate(x=SO_cont2$OrderValue,by=list(SO_cont2$BuyerID), FUN=sum)
SVB <- rbind(SVB_treat,SVB_cont2)
t.test(SVB_treat$x,SVB_cont2$x)
ate <- mean(SVB_treat$x) - mean(SVB_cont2$x)
treat <- c(rep(1,length(SVB_treat$x)),rep(0,length(SVB_cont2$x)))
dist <- rep(0,100000)
for (i in 1:100000){
  treatment <- sample(treat)
  dist[i] <- mean(SVB$x[treatment==1])-mean(SVB$x[treatment==0])
...
sum(abs(dist)>=abs(ate))/length(dist)
sd(dist)
hist(SVB$x, breaks=10)
```

### 3.5 Sales Quantity per Buyer

```{r}
## Contrl 1 v.s. Treatment
...
plot(density(dist), main="Sharp null hypothesis dist. of Sales (TR vs C1)")
abline(v=ate, col="red")
sum(abs(dist)>=abs(ate))/length(dist) # p-value
sd(dist) # standard error
hist(SQB$x)

## Treatment v.s. Control 2
SQB_treat <- aggregate(x=SO_treat$NrItems,by=list(SO_treat$BuyerID), FUN=sum)
SQB_cont2 <- aggregate(x=SO_cont2$NrItems,by=list(SO_cont2$BuyerID), FUN=sum)
SQB <- rbind(SQB_treat,SQB_cont2)
t.test(SQB_treat$x,SQB_cont2$x)
ate <- mean(SQB_treat$x) - mean(SQB_cont2$x)
treat <- c(rep(1,length(SQB_treat$x)),rep(0,length(SQB_cont2$x)))
dist <- rep(0,100000)
for (i in 1:100000){
  treatment <- sample(treat)
  dist[i] <- mean(SQB$x[treatment==1])-mean(SQB$x[treatment==0])
...
## Control 1 v.s. Treatment
conversion <- c(conversion_cont1,conversion_treat)
t.test(conversion_cont1,conversion_treat)
wilcox.test(conversion_cont1,conversion_treat)
ate <- mean(conversion_treat)-mean(conversion_cont1)
treat <- c(rep(0,length(conversion_cont1)),
           rep(1,length(conversion_treat)))
dist <- rep(0,100000)
for (i in 1:100000){
  treatment <- sample(treat)
  dist[i] <- mean(conversion[treatment==1])-mean(conversion[treatment==0])
}
plot(density(dist),
     main="Sharp null hypothesis dist. of Conversion Rate (TR vs C1)")
abline(v=ate, col="red")
sum(abs(dist)>=abs(ate))/length(dist)# p-value
sd(dist) # standard error
...
## Treatment v.s. Control 2
conversion <- c(conversion_cont2,conversion_treat)
t.test(conversion_cont2,conversion_treat)
wilcox.test(conversion_cont2,conversion_treat)
ate <- mean(conversion_treat)-mean(conversion_cont2)
treat <- c(rep(0,length(conversion_cont2)),
           rep(1,length(conversion_treat)))
dist <- rep(0,100000)
for (i in 1:100000){
  treatment <- sample(treat)
...
plot(density(dist),
     main="Sharp null hypothesis dist. of Conversion Rate (TR vs C2)")
abline(v=ate, col="red")
sum(abs(dist)>=abs(ate))/length(dist)# p-value
sd(dist) # standard error


## Control 1 v.s. Control 2 - Baseline check
mean(conversion_cont1)-mean(conversion_cont2)
t.test(conversion_cont1,conversion_cont2)
wilcox.test(conversion_cont1,conversion_cont2)
```

### 3.7 Overall Revenue Compared to Past Half Year
...
* Differences in Sales Order Value (SO) is NOT significant across treatment and control weeks.

* Differences in Order Value per Buyer (OVB) and Order QTY per Buyer (OQB) are both NOT significant.  

* Differences in Sales per Unique Item per Order (SIO) IS significant. The lower value in treatment week marks that customers tends to buy lower value items during the treatment is offered.

* Differences in Conversion Rate is NOT significant. The average across the weeks are similar, marking the no effect of treatment on the likelihood of transforming web sessions into purchases.
